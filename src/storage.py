...
def safe_json_dumps(obj: Any) -> Optional[str]:
    ...
    Returns:
        JSON string if serialization successful, None if obj is None or
        serialization fails
    """
    if obj is None:
        return None
    ...
    try:
        ...
        logger.warning(
            f"Failed to serialize object to JSON: {e}. Object type: {type(obj)}"
        )
        return None
...
class StorageManager:
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_story_activity_muting
    ...
    def update_story_media(
        self, story_id: str, local_path: str, file_size: int
    ) -> bool:
    ...
    def save_account_story_activity(
    ...
    def update_account_story_activity(self, user_id: str, **kwargs) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                query = (
                    f"UPDATE account_story_activity "
                    f"SET {', '.join(update_fields)} WHERE user_id = ?"
                )
                cursor.execute(query, values)
                
                if cursor.rowcount == 0:
                    logger.warning(
                        f"No story activity record found to update for user {user_id}"
                    )
                    return False
    ...
    def get_account_story_activity(
        self, user_id: str
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT * FROM account_story_activity WHERE user_id = ?",
                    (user_id,)
                )
                row = cursor.fetchone()
                return dict(row) if row else None
    ...
    def get_unmuted_accounts_for_stories(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                accounts = [dict(row) for row in cursor.fetchall()]
                logger.debug(
                    f"Retrieved {len(accounts)} unmuted accounts for stories"
                )
                return accounts
        except Exception as e:
    ...
    def get_accounts_by_story_priority(
        self, priority: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get accounts by story priority {priority}: {e}"
            )
            return []
    ...
    def get_story_priority_distribution(self) -> Dict[str, int]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """)
                distribution = {
                    row['story_poll_priority']: row['count']
