...
This module generates valid RSS 2.0 XML feeds from stored Instagram posts and
stories, with support for embedded images, videos, carousel posts, and story
text content (polls, links, stickers).
"""
...
class RSSGenerator:
    ...
    def generate_feed(self, posts: List[Dict[str, Any]],
        ...
        Posts and stories are combined into a single feed, sorted by date
        (newest first). Story items are prefixed with [STORY] in their titles.
        ...
        stories = stories or []
        logger.info(
            f"Generating RSS feed with {len(posts)} posts and "
        ...
        # Combine posts and stories, mark type for sorting
        ...
        all_items.sort(
            key=lambda x: x['data'].get(
                'taken_at' if x['type'] == 'story' else 'posted_at'
            ),
            reverse=True
        )
        
        # Add items for each post/story
        for feed_item in all_items:
            if feed_item['type'] == 'post':
                self._add_post_item(channel, feed_item['data'])
            else:
                self._add_story_item(channel, feed_item['data'])
    ...
    def _add_story_item(self, channel: ET.Element, story: Dict[str, Any]):
        ...
        ET.SubElement(item, 'author').text = (
            f"{story['username']}@instagram.com ({author_name})"
        )
    ...
    def _format_story_description(self, story: Dict[str, Any]) -> str:
        ...
        if media_type == 'image':
            html_parts.append(
                f'<p><img src="{html.escape(media_url)}" '
                f'style="max-width:100%;height:auto;" /></p>'
            )
        elif media_type == 'video':
            html_parts.append(
                f'<p><video controls style="max-width:100%;height:auto;">'
                f'<source src="{html.escape(media_url)}" type="video/mp4" />'
                f'Your browser does not support the video tag.</video></p>'
            )
        ...
        if story.get('poll_question'):
            text_parts.append(
                f"<strong>Poll:</strong> {html.escape(story['poll_question'])}"
            )
            if story.get('poll_options'):
                try:
                    ...
                    options_html = '<br/>'.join(
                        [f"&bull; {html.escape(opt)}" for opt in options]
                    )
        ...
        if story.get('link_text'):
            text_parts.append(
                f"<strong>Link:</strong> {html.escape(story['link_text'])}"
            )
        ...
        if story.get('sticker_text'):
            try:
                ...
                if sticker_data:
                    text_parts.append(
                        f"<strong>Stickers:</strong> "
                        f"{html.escape(str(sticker_data))}"
                    )
            except (json.JSONDecodeError, TypeError):
                pass
        ...
        if expires_at:
            ...
            html_parts.append(
                f'<p><em>Story expires: '
                f'{expires_at.strftime("%Y-%m-%d %H:%M UTC")}</em></p>'
            )
        
        # Add link to original story
        permalink = html.escape(story["permalink"])
        html_parts.append(
            f'<p><a href="{permalink}">View on Instagram</a></p>'
        )
