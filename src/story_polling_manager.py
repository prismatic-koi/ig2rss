...
class StoryPollingManager:
    ...
    def __init__(
        ...
        Args:
            ...
            poll_high_every_n: Poll high priority every N cycles (default: 1)
            poll_normal_every_n: Poll normal priority every N cycles (default: 1)
            poll_low_every_n: Poll low priority every N cycles (default: 3)
            poll_dormant_every_n: Poll dormant every N cycles (default: 12)
    ...
    def _load_cycle_number(self) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Invalid story cycle number '{cycle_str}', resetting to 0"
            )
            return 0
    ...
    def increment_cycle(self) -> int:
        ...
        self.storage.save_sync_metadata(
            'story_cycle_number', str(self.current_cycle)
        )
    ...
    def is_first_sync(self) -> bool:
        ...
        """
        initialized = self.storage.get_sync_metadata(
    ...
    def initialize_story_activity_profiles(
        ...
        Checks mute status for each account and sets conservative initial
        priority. Propagates InstagramChallengeError to the caller so the
        ...
        """
        logger.info(
            f"Initializing story activity profiles for {len(accounts)} accounts"
        )

        now = datetime.now()
        distribution: Dict[str, int] = {
            'high': 0, 'normal': 0, 'low': 0, 'dormant': 0, 'muted': 0
        }

        for account in accounts:
            # Check if stories are muted for this account
            # InstagramChallengeError will propagate up
            is_muted = self.instagram_client.check_story_mute_status(
                account.user_id
            )
            ...
            # Start with conservative 'normal' priority for all non-muted
            priority = 'normal'
            ...
            logger.debug(
                f"Initialized @{account.username}: "
                f"muted={is_muted}, priority={priority}"
            )
    ...
    def get_accounts_to_poll_this_cycle(
        ...
        Only returns unmuted accounts that are due for polling based on
        their priority.
        ...
        for activity in unmuted_accounts:
            ...
            if priority == 'high':
                ...
                logger.warning(
                    f"Unknown priority '{priority}' for {activity['username']}"
                )
                continue
        ...
        logger.info(
            f"Cycle {cycle}: Polling {len(accounts_to_poll)} accounts for "
            f"stories (total unmuted: {len(unmuted_accounts)})"
        )
    ...
    def update_account_priority_after_check(
        ...
        Args:
            ...
            story_metadata: Metadata from story check
        ...
        if has_new_stories:
            ...
            update_kwargs: Dict[str, Any] = {
                ...
                'stories_fetched_count': (
                    activity['stories_fetched_count']
                    + story_metadata.get('story_count', 0)
                ),
            }
        else:
        ...
        new_priority = self._refine_priority(
            activity, new_consecutive, has_new_stories
        )
        ...
        logger.debug(
            f"@{username}: priority="
            f"{activity['story_poll_priority']}->{new_priority}, "
    ...
    def _refine_priority(
        ...
        Args:
            ...
            consecutive_no_new: Consecutive checks with no new stories
            has_new_stories: Whether new stories were found
        ...
        # Use naive datetime.now() to match the rest of the codebase.
        # All timestamps stored via the SQLite adapter are naive local time.
        now = datetime.now()
    ...
    def should_refresh_mute_statuses(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Invalid last_mute_refresh value: {last_refresh_str}"
            )
            return True
    ...
    def refresh_mute_statuses(self) -> Tuple[int, int]:
        ...
        for activity in all_activities:
            ...
            # InstagramChallengeError will propagate up
            new_muted = self.instagram_client.check_story_mute_status(
                activity['user_id']
            )
            ...
            if old_muted != new_muted:
                if new_muted:
                    newly_muted += 1
                    logger.info(
                        f"@{activity['username']}: Stories newly muted"
                    )
                else:
                    newly_unmuted += 1
                    logger.info(
                        f"@{activity['username']}: Stories newly unmuted"
                    )
    ...
    def get_priority_stats(self) -> Dict[str, Any]:
        ...
        all_activities = self.storage.get_all_account_story_activity()
        muted_count = sum(
            1 for a in all_activities if a['is_muting_stories']
        )
